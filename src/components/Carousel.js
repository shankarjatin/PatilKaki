...
const Carousel = () => {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden mt-10">
      <div className="flex transition-transform ease-out duration-500" style={{ transform: `translateX(-${current * 100}%)` }}>
        {cards.map((card) => (
          <div key={card.id} className="min-w-full bg-purple-100 flex items-center justify-center h-full p-4">
            <div className="w-full max-w-screen-lg bg-gray-100 h-250 bg-custom-blue rounded-3xl flex mx-8">
      ...
      <button onClick={prevCard} className="absolute w-20 h-20 left-20 top-1/3 transform -translate-y-1/2 bg-gray-100 text-white ml-8 border-purple-400 border-2 rounded-full">
        <span className='text-2xl text-purple-400'>&lt;</span>
      </button>
      <button onClick={prevCard} className="absolute w-20 h-20 right-20 top-1/3 transform -translate-y-1/2 bg-gray-100 text-white mr-8 border-purple-400 border-2 rounded-full">
        <span className='text-2xl text-purple-400'>&gt;</span>
      </button>
    </div>
  );
